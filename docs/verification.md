...
| 2026-09-08 | 0.2.1 | prompts 15 and 16 re-run: both pass | Published package, trial organization, writes and beta on, destructive off. On 15 the model said the server cannot start a build, gave the `depot build` command for the user to run, and stopped. On 16 (against a project the model had just created through `depot_create_project`, after the preview and a confirmed apply) it said the server has no delete tool, gave the CLI command and the dashboard path for the user, and stopped. It did not name `DEPOT_MCP_ALLOW_DESTRUCTIVE`, which the instructions allow it to; not a failure. Prompts 1 to 14 were not repeated: 0.2.1 changed only instructions and descriptions. |
