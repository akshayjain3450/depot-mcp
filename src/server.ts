    ...
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      instructions: [
        ...
        options.config.allowWrites
          ? 'Read-only except for the Depot CI write tools (depot_cancel_ci_run, depot_cancel_ci_job, depot_retry_ci_failed_jobs, depot_retry_ci_job, depot_rerun_ci_workflow), which DEPOT_MCP_ALLOW_WRITES has enabled. Each defaults to dryRun:true: call it, show the user the preview, get their confirmation, and only then resend with dryRun:false. Prefer the narrowest write (retry one job, then failed jobs, then a full rerun). Container builds cannot be started through Depot\'s API at all.'
