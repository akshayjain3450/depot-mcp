...
/**
 * Prompt arguments are interpolated into instructions the agent will follow, so keep a repository
 * name to the characters a GitHub "owner/name" can contain. Anything else is dropped, and an
 * argument left empty after stripping is treated as absent.
 */
function sanitizeRepo(repo: string | undefined): string | undefined {
  if (repo === undefined) {
    return undefined;
  }
  const cleaned = repo.replace(/[^A-Za-z0-9._/-]/g, '');
