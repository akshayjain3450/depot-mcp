...
- README, package metadata, the Docker image label, the server instructions, SECURITY.md, and CONTRIBUTING.md describe the server as read-only by default with opt-in write tools, instead of read-only. The README's status bullets, Agent Skills comparison, token matrix, write-tool table, and security section each carried two or three versions of the same line from successive write releases; each is now one line that matches the registered tools, and the verification note records that every write except sandbox stop and kill has been applied live.
...
- Eight write tools behind `DEPOT_MCP_ALLOW_WRITES`, absent from `tools/list` otherwise: `depot_cancel_ci_run`, `depot_cancel_ci_job`, `depot_retry_ci_failed_jobs`, `depot_retry_ci_job`, `depot_rerun_ci_workflow`, `depot_set_ci_variable`, `depot_delete_ci_variable`, `depot_create_project`. Each defaults to `dryRun: true`, previews from Depot's read RPCs, refuses unsafe preconditions before any mutating RPC, logs one audit line to stderr per applied write, and carries honest `destructiveHint` and `idempotentHint` values. Verified live against a trial organization on 2026-09-07: set and delete variable, rerun workflow, create project (apply path); every other write's dry-run path. `RerunWorkflow` resets the same workflow rather than creating a new run.
...
- `depot_retry_ci_job` and `depot_cancel_ci_job` send the workflow id alongside the job id, as Depot's `RetryJobRequest` and `CancelJobRequest` require; without it Depot answered `400 Workflow not found`. Found by the first live apply run.
