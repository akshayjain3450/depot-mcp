...
describe('dist/index.js over stdio: configuration and protocol details', () => {
  ...
  it('announces the registered write tools on stderr when DEPOT_MCP_ALLOW_WRITES is set', async () => {
    ...
    expect(exit.code).toBe(0);
    expect(spawned.stderr()).toContain('28 read-only tool(s), 0 beta tool(s), 9 mutating tool(s), 0 of them destructive');
  ...
  it('announces the destructive tool on stderr only when both write flags are set', async () => {
    ...
    const exit = await withTimeout(spawned.exited, 2_000, 'exit after stdin closed');
    expect(exit.code).toBe(0);
    expect(spawned.stderr()).toContain('28 read-only tool(s), 0 beta tool(s), 10 mutating tool(s), 1 of them destructive');
