...
describe('dist/index.js over stdio: configuration and protocol details', () => {
  ...
  it('names the registered write tools on stderr when DEPOT_MCP_ALLOW_WRITES is set', async () => {
    ...
    expect(exit.code).toBe(0);
    expect(spawned.stderr()).toContain('16 read-only tool(s), 3 mutating tool(s)');
    expect(spawned.stderr()).toContain('depot_set_ci_variable, depot_delete_ci_variable, depot_create_project');
